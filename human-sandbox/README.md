...
The sandbox stays isolated from its environment: no host `~/.aws` inside. Instead the entrypoint derives the aws `cloudflare` profile that tofu's s3 state backend needs from the project's `CLOUDFLARE_API_TOKEN` (R2 S3 creds are the token id and the sha256 of its value, per the [cloudflare docs](https://developers.cloudflare.com/r2/api/tokens/#get-s3-api-credentials-from-an-api-token)), pinned to the right account by `CLOUDFLARE_ACCOUNT_ID`. It also writes a `default` profile mirroring the selected host profile's SSO settings (mined by `up.bash` with `aws configure get`, same as the `infisical_*` keys), so `aws sso login` works inside: with no browser the CLI prints the authorization URL and code to open on any logged-in device. Host profiles without SSO settings start with a warning and no default profile. `tofu` is installed and `TF_DATA_DIR` is `.sandbox_tofu`: provider downloads stay separate from the host's `.terraform` (darwin binaries the container can't run) and persist across sessions on the mounted workspace.
