...
description: Typecheck pi sandbox extensions before committing — run tsc --noEmit in pi-sandbox/container/extensions after any .ts change. Load before editing or creating extension files.
---

# Extension typecheck discipline

The `pi-sandbox/container/extensions/` directory contains TypeScript files
(pi extensions) that are never compiled or typechecked by the runtime —
...
`pi-sandbox/container/extensions/` **before committing** any change to a
...
```bash
cd pi-sandbox/container/extensions
...
- **Wrong package imports** — `@earendel-works/pi-coding-agent` (typo) and
  `@oh-my-pi/pi-coding-agent` (the old fork) don't resolve;
  `@earendil-works/pi-coding-agent` is the real package. tsc catches
