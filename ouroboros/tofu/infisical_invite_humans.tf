# This spagetti is honestly only for inviting the human admins to the newly created organization.
# It's such a minor thing but it was driving me insane.

resource "infisical_identity" "human_inviter" {
  name   = "human-inviter"
  role   = "member"
  org_id = local.org_id
}

resource "infisical_identity_token_auth" "human_inviter" {
  identity_id = infisical_identity.human_inviter.id
}

resource "infisical_identity_token_auth_token" "human_inviter" {
  identity_id = infisical_identity.human_inviter.id
  name        = "human-inviter"
  depends_on  = [infisical_identity_token_auth.human_inviter]
}

data "http" "human_list" {
...
locals {
  user_list_raw = jsondecode(nonsensitive(data.http.human_list.response_body))
  user_list_parsed = {
...
resource "restapi_object" "invite_human_admins" {
  provider  = restapi.infisical_project_memberships
  for_each  = local.org_admins
  path      = "/projects/${infisical_project.ouroboros.id}/memberships"
  ...
  id_attribute = "id"
  read_path    = "/projects/${infisical_project.ouroboros.id}/memberships"
  read_search = {
  ...
  destroy_path = "/projects/${infisical_project.ouroboros.id}/memberships"
  ...
  force_new = [
    infisical_project.ouroboros.id,
    each.value.id,
    each.key,
    "admin",
  ]
  depends_on = [infisical_project_identity.human_inviter]
}
