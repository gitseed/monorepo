...
main() {
    ...
    # settings, so `aws sso login` works inside without a host browser: the
    # CLI prints the authorization URL and code for the user to open
    # elsewhere. Mined with `aws configure get` like the infisical keys,
    # except sso-session sections: configure get can't read those (they are
    # not profiles), so they fall back to parsing the config file. The
    # sandbox writes its config from these at startup; unset keys pass
    # through as empty and are skipped there.
    sso_session_value() {
        awk -v name="$1" '
